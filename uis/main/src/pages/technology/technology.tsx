...
const SmallCard = ({ title, para, img, className = "" }) => {
  // Debug log to see what classes are being applied
  console.log('SmallCard className:', className);
  
  return (
    <div
      className={`w-full flex flex-col gap-[3px] max-w-[273px] h-[300px] 
      bg-[#F9F9F933] relative rounded-[1rem] shadow-md pt-[88px] px-[32px] pb-[32px] 
      small-card ${className}`}
    >
      {/* Floating image */}
      <img
        src={img}
...
const useIntersectionObserver = (ref, options = {}) => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        setIsVisible(entry.isIntersecting); // This will reset when out of view
      },
      {
        threshold: 0.1,
...
// SmallCard Grid Component with Animation
const SmallCardGrid = ({ cards, image, animationClasses }) => {
      ...
      entries.forEach((entry) => {
        ...
        if (index !== -1) {
          setVisibleCards(prev => {
            const newState = [...prev];
            newState[index] = entry.isIntersecting; // Reset when out of view
            return newState;
          });
        }
      });
...
const Technology1 = () => {
  ...
  const [visibleRows, setVisibleRows] = useState([false, false]);
  const firstRowRef = useRef(null);
  const secondRowRef = useRef(null);
  const roiRef = useRef(null);
  const roiVisible = useIntersectionObserver(roiRef);
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.target === firstRowRef.current) {
            setVisibleRows((prev) => [entry.isIntersecting, prev[1]]);
          } else if (entry.target === secondRowRef.current) {
            setVisibleRows((prev) => [prev[0], entry.isIntersecting]);
          }
        });
      },
      ...
      <div className="w-full bg-[linear-gradient(to_bottom,_#020919,_#000C28,_#000416,_#000820,_#00092E,_#000523,_#0944CF)] flex flex-col p-[120px] gap-[120px]">
        ...
        <div className="relative w-screen left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] flex flex-col gap-[36px]">
          ...
          <div className="w-full flex flex-col max-w-[1200px] h-[746px] pt-10 mx-auto sm:px-6 lg:px-0 gap-[76px]">
            ...
            <div 
              ref={firstRowRef}
              className="grid grid-cols-1 lg:grid-cols-2 gap-[36px]"
            >
              {cardData.slice(0, 2).map((card, index) => (
                <div 
                  ...
                  className={`
                    w-full card-slide
                    ${index === 0 ? 'card-slide-left' : 'card-slide-right'}
                    ${visibleRows[0] ? 'visible' : ''}
                  `}
        ...
        <div className="relative w-screen left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] flex flex-col gap-[36px]">
          ...
          <div className="w-full pt-10 max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-0 gap-[36px]">
            <SmallCardGrid
              cards={scardmap}
              image={inductor}
              animationClasses={['slideFromLeft', 'slideFromBottom', 'slideFromBottom', 'slideFromRight']}
            />
          </div>
        </div>
        ...
        <div className="w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-0 mb-20">
          <div className="relative w-screen left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] flex flex-col gap-[36px]">
            ...
            <div 
              ref={roiRef}
              className={`bg-[#F9F9F933] flex flex-col gap-[3px] mx-auto relative rounded-[2rem] shadow-md p-[32px] w-full max-w-[586px] h-[280px] small-card slideFromBottom ${
                roiVisible ? 'visible' : ''
              }`}
